...
export function setupMonitoring(app: FastifyInstance) {
    ...
    app.get('/api/monitor/performance', async () => {
        ...
        return {
            ...
            messagesPerSecond: parseFloat(stats.messagesPerSecond.toFixed(2)),
            avgLatency: parseFloat(stats.avgLatency.toFixed(2)),
            p50Latency: parseFloat(stats.p50Latency.toFixed(2)),
            p95Latency: parseFloat(stats.p95Latency.toFixed(2)),
            p99Latency: parseFloat(stats.p99Latency.toFixed(2)),
            errorRate: parseFloat((stats.errorRate * 100).toFixed(2)),
            cacheHitRate: parseFloat((stats.cacheHitRate * 100).toFixed(2)),
            memoryUsageMB: parseFloat(stats.memoryUsageMB.toFixed(2)),
        };
    });
