...
const isValidUin = (value) => /^\d{5,11}$/.test(value);
...
const extractReplySenderId = (event) => {
    const raw = event.raw || {};
    ...
    const replySegment = segments.find((seg) => !!seg && seg.type === 'reply');
    const replyData = replySegment?.data;
    if (replyData?.senderId)
        return String(replyData.senderId);
    if (replyData?.userId)
        return String(replyData.userId);
    return null;
};
const hasReplyMessage = (event) => {
    const raw = event.raw || {};
    ...
    if (raw.rawReply)
        return true;
    const segments = Array.isArray(event.message?.segments) ? event.message.segments : [];
    return segments.some((seg) => seg && seg.type === 'reply');
};
...
const parseUserAction = (args, event, hasReply) => {
    let uin = null;
    let action = 'toggle';
    if (hasReply) {
...
const parseUserContent = (args, event, hasReply) => {
    let uin = null;
    let contentParts = [];
    if (hasReply) {
        uin = extractReplySenderId(event);
...
const getMemberInfoSafe = async (qqClient, groupId, userId) => {
    try {
...
const isGroupAdmin = async (qqClient, groupId, userId) => {
    const memberInfo = await getMemberInfoSafe(qqClient, groupId, userId);
    if (!memberInfo)
        return false;
    return memberInfo.role === 'admin' || memberInfo.role === 'owner';
};
const isGroupOwner = async (qqClient, groupId, userId) => {
    const memberInfo = await getMemberInfoSafe(qqClient, groupId, userId);
    if (!memberInfo)
        return false;
    return memberInfo.role === 'owner';
};
const canManageUser = async (qqClient, groupId, operatorId, targetId) => {
...
const plugin = {
    ...
    install: async (ctx, _config) => {
        ...
        const ensureTelegram = (event) => event.platform === 'tg';
        const resolveBinding = async (event) => {
        ...
        const ensureTargetUin = (event, args, argIndex) => {
        ...
        const handleUnban = async (event, args) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
            const targetUin = ensureTargetUin(event, args, 0);
        ...
        const handleKick = async (event, args) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
            const targetUin = ensureTargetUin(event, args, 0);
        ...
        const handleCard = async (event, args) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
        ...
        const handleMuteAll = async (event, args, commandName) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
            const qqClient = event.qq;
        ...
        const handleAdmin = async (event, args) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
            const qqClient = event.qq;
        ...
        const handleGroupName = async (event, args) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
            const qqClient = event.qq;
        ...
        const handleTitle = async (event, args) => {
            const binding = await resolveBinding(event);
            if (!binding)
                return;
            const qqClient = event.qq;
