...
const normalizeFlagName = (input) => String(input || '').trim().replace(/\s+/g, '_');
const loadAllFlags = async (ctx) => {
    try {
...
const saveAllFlags = async (ctx, data) => {
    await ctx.storage.set(STORAGE_KEY, data);
};
const applyInstanceFlags = (event, flags) => {
...
const plugin = {
    ...
    install: async (ctx, _config) => {
        ...
        const setFlag = async (event, flagName, enabled) => {
            try {
                const instanceId = String(event.instance?.id ?? event.instanceId ?? '0');
                const allFlags = await loadAllFlags(ctx);
                ...
                await event.reply(`${status} 功能标志: \`${flagName}\`\n\n✅ 已持久化保存（实例 ${instanceId}）`);
            }
            catch (error) {
