...
export class AuthService {
     ...
     * 用户名密码登录
     */
    static async loginWithPassword(username, password, ipAddress, userAgent) {
        ...
        // 创建 session token
        const token = await TokenManager.createSessionToken(user.id, ipAddress, userAgent);
        // 记录审计日志
     ...
     * 创建管理员用户
     */
    static async createAdminUser(username, password, displayName, email, createdBy) {
     ...
     * 记录审计日志
     */
    static async logAudit(userId, action, resource, resourceId, details, ipAddress, userAgent) {
