...
import { setGlobalRuntime } from '@napgram/runtime-kit';
 ...
 * 插件运行时公共 API
 */
export class PluginRuntimeAPI {
    static instance;
    webRoutes;
    instanceResolver;
    instancesResolver;
    builtins = [];
    setWebRoutes(register) {
        this.webRoutes = register;
    }
    setInstanceResolvers(instanceResolver, instancesResolver) {
        this.instanceResolver = instanceResolver;
        this.instancesResolver = instancesResolver;
    }
    async reloadCommandsForInstances() {
        if (!this.instancesResolver)
            return;
        const instances = this.instancesResolver();
        for (const instance of instances) {
            try {
                const featureManager = instance?.featureManager; // Using any as IInstance doesn't define featureManager yet
    ...
    configureApis() {
        const instanceResolver = this.instanceResolver || ((_id) => undefined);
