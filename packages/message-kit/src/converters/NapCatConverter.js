...
export class NapCatConverter extends BaseConverter {
    textConverter = new TextSegmentConverter();
    mediaConverter = new MediaSegmentConverter();
    interactionConverter = new InteractionSegmentConverter();
    jsonCardConverter = new JsonCardConverter();
    /**
     * 从 NapCat 消息转换为统一格式
     */
    ...
    convertNapCatSegment(segment, rawMsg) {
        this.logger.debug(`Converting segment:\n${JSON.stringify(segment, null, 2)}`);
        const data = segment?.data || {};
        const type = (segment?.type || '');
        const rawMessage = rawMsg?.raw_message;
        switch (type) {
            // Text types
            case 'text':
