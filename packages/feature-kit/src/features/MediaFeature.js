...
export class MediaFeature {
                ...
                if (fileUrl) {
                    ...
                    if (/^https?:\/\//.test(fileUrl)) {
                        try {
                            ...
                            const downloaded = typeof qq.downloadFile === 'function'
     ...
     * 创建临时文件
     */
    async createTempFileFromBuffer(buffer, extension = '.tmp') {
     ...
     * 检查媒体大小是否超限
     */
    isMediaTooLarge(buffer, maxSize = 20 * 1024 * 1024) {
        return buffer.length > maxSize;
    }
    /**
     * 压缩图片（如果需要）
     */
    async compressImage(buffer, maxSize = 5 * 1024 * 1024) {
        try {
            // 如果文件已经小于限制，直接返回
