...
export class ForwardMediaPreparer {
        ...
        await Promise.all(msg.content.map(async (content) => {
            try {
                if (content.type === 'image') {
                    ...
                    content.data.file = await this.ensureFilePath(await this.ensureBufferOrPath(content, { prefer: 'url', ext: '.jpg', prefix: 'tg-image' }), '.jpg');
                }
                else if (content.type === 'video') {
                    content.data.file = await this.ensureFilePath(await this.ensureBufferOrPath(content, { prefer: 'url', ext: '.mp4', prefix: 'tg-video' }), '.mp4', false);
                }
                else if (content.type === 'audio') {
                    const oggPath = await this.ensureFilePath(await this.ensureBufferOrPath(content, { forceDownload: true, prefer: 'path', ext: '.ogg', prefix: 'tg-audio' }), '.ogg', true);
                    if (oggPath) {
                        try {
                    ...
                    const file = content;
                    content.data.file = await this.ensureFilePath(await this.ensureBufferOrPath(file, { prefer: 'url', filename: file.data.filename, prefix: 'tg-file' }), undefined);
                }
            }
            catch (err) {
                this.logger.warn(err, 'Prepare media for QQ failed, skip media content:');
                content.type = 'text';
                content.data = { text: this.contentRenderer(content) };
            }
        }));
    }
    async ensureBufferOrPath(content, options) {
        ...
        if (content.data.file) {
            ...
            try {
                const mediaObj = content.data.file;
    ...
    async ensureFilePath(file, ext, forceLocal) {
        if (!file)
            return undefined;
    ...
    async prepareAudioSource(audioContent, processedFile) {
        let source = processedFile;
    ...
    async waitFileStable(filePath, attempts = 3, intervalMs = 150) {
        if (!filePath)
            return false;
        let lastSize = -1;
    ...
    async convertAudioToOgg(source) {
        const tempDir = path.join(env.DATA_DIR, 'temp');
