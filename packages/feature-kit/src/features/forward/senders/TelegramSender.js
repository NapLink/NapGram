...
export class TelegramSender {
    ...
    media;
    contentRenderer;
    logger = getLogger('ForwardFeature');
    audioConverter;
    fileNormalizer;
    richHeaderBuilder;
    mediaSender;
    constructor(instance, media, contentRenderer = renderContent) {
    ...
    async sendToTelegram(chat, msg, pair, replyToMsgId, nicknameMode) {
        ...
        const flushMediaBatch = async () => {
            if (mediaBatch.length > 0) {
                ...
                lastSent = await this.mediaSender.sendMediaGroup(chat, mediaBatch, captionStr, replyToMsgId, pair, originalHeader, // Use original header for media
                richHeaderUsed, richHeaderUrl, msg.id, this.sendMediaToTG.bind(this)) || lastSent;
                mediaBatch.length = 0;
    ...
    async sendMediaToTG(chat, header, content, replyToMsgId, pair, richHeaderUsed, richHeaderUrl, qqMsgId) {
        let fileSrc;
        try {
        ...
        try {
            ...
            if (content.type === 'image') {
                const fileName = content.data.fileName || (typeof content.data.file === 'string' ? path.basename(content.data.file) : 'image.jpg');
                ...
                const fileName = content.data.fileName || (typeof content.data.file === 'string' ? path.basename(content.data.file) : 'video.mp4');
                ...
                const fileName = content.data.fileName
                    || (typeof content.data.file === 'string' ? path.basename(content.data.file).replace(/\.amr$/, '.ogg') : 'audio.ogg');
            ...
            if (mediaInput) {
                ...
                if (ttlSeconds && typeof mediaInput.type === 'string' && ttlAllowedTypes.has(mediaInput.type)) {
                    mediaInput.ttlSeconds = ttlSeconds;
                }
                const params = {
                    ...commonParams,
                    ...formattingParams,
    ...
    async sendForwardToTG(chat, content, pair, replyToMsgId, header = '', richHeaderUsed) {
