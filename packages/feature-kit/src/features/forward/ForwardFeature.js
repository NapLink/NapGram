...
export class ForwardFeature {
    ...
    handleTgMessage = async (tgMsg) => {
        ...
        const threadId = new ThreadIdExtractor().extractFromRaw(tgMsg.raw || tgMsg);
        const pair = this.forwardMap.findByTG(tgMsg.chat.id, threadId, !threadId);
        if (!pair) {
        ...
        let unified;
        try {
            unified = messageConverter.fromTelegram(tgMsg);
        }
        catch (e) {
        ...
        try {
            const gatewayMessage = unified ?? messageConverter.fromTelegram(tgMsg);
            await this.instance.eventPublisher?.publishMessageCreated(this.instance.id, gatewayMessage, pair);
        }
        catch (e) {
    ...
    constructor(instance, tgBot, qqClient, media, commands) {
        ...
        const isForwardMap = pairs && typeof pairs.findByQQ === 'function' && typeof pairs.findByTG === 'function';
        if (!isForwardMap) {
            throw new Error('Forward map is not initialized for NapCat pipeline.');
        }
        this.forwardMap = pairs;
        ...
        this.mediaGroupHandler = new MediaGroupHandler(this.qqClient, msg => this.mediaPreparer.prepareMediaForQQ(msg), pair => this.getNicknameMode(pair));
        this.tgMessageHandler = new TelegramMessageHandler(this.qqClient, this.mediaGroupHandler, this.replyResolver, msg => this.mediaPreparer.prepareMediaForQQ(msg), this.renderContent.bind(this), pair => this.getNicknameMode(pair));
        this.setupListeners();
     ...
     * 优先使用 pair 的配置，若为 null 则使用环境变量默认值
     */
    getNicknameMode(pair) {
        return pair.nicknameMode || env.SHOW_NICKNAME_MODE;
    }
    toPluginSegments(contents, platform) {
        const out = [];
        for (const c of contents || []) {
            ...
            switch (c.type) {
                case 'text':
                    out.push({ type: 'text', data: { text: String(c.data?.text ?? '') } });
                    break;
                case 'at':
                    out.push({
                        type: 'at',
                        data: {
                            ...
                            userName: c.data?.userName ? String(c.data.userName) : undefined,
                        },
                    });
                    break;
                case 'reply':
                    out.push({ type: 'reply', data: { messageId: String(c.data?.messageId ?? '') } });
                    break;
                case 'image': {
                    const data = c.data;
                    out.push({
                ...
                case 'forward': {
                    const msgs = Array.isArray(c.data?.messages) ? c.data.messages : [];
                    out.push({
                        type: 'forward',
                        data: {
                            messages: msgs.map((m) => ({
    ...
    async publishTgPluginEvent(tgMsg, pair, unified, threadId) {
        try {
            const eventPublisher = getEventPublisher();
            const message = unified ?? messageConverter.fromTelegram(tgMsg);
            const segments = this.toPluginSegments(message.content, 'tg');
            ...
            eventPublisher.publishMessage({
                ...
                reply: async (content) => {
                    ...
                    const params = { replyTo: tgMsg.id };
                    if (threadId)
                        params.messageThreadId = threadId;
                    const sent = await chat.sendMessage(replyText, params);
                    return { messageId: `tg:${String(tgMsg.chat.id)}:${String(sent?.id ?? '')}` };
                },
                send: async (content) => {
                    const chat = await this.tgBot.getChat(Number(tgMsg.chat.id));
    ...
    handleQQMessage = async (msg) => {
        ...
        try {
            ...
            try {
                const eventPublisher = getEventPublisher();
                const channelType = msg.chat.type === 'private'
                    ? 'private'
                    : msg.chat.type === 'group'
                        ? 'group'
                        : 'group';
                const segments = this.toPluginSegments(msg.content, 'qq');
                eventPublisher.publishMessage({
            ...
            // Publish gateway event (doesn't affect forwarding)
            try {
                await this.instance.eventPublisher?.publishMessageCreated(this.instance.id, msg, pair);
            }
            catch (e) {
    ...
    handleModeCommand = async (msg, args) => {
    ...
    handlePokeEvent = async (groupId, operatorId, targetId) => {
        try {
