...
export class ForwardMapper {
    ...
    constructor(contentRenderer = renderContent) {
    ...
    async saveTgToQqMapping(unified, tgMsg, receipt, pair) {
    ...
    async saveMessage(qqMsg, tgMsg, instanceId, qqRoomId, tgChatId) {
        if (this.shouldSkipPersistence()) {
            return;
        }
        try {
    ...
    async findTgMsgId(instanceId, qqRoomId, qqMsgId) {
    ...
    async findQqSource(instanceId, tgChatId, tgMsgId) {
        if (this.shouldSkipPersistence()) {
            return undefined;
        }
