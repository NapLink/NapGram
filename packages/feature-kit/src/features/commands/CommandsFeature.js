...
export class CommandsFeature {
    ...
    constructor(instance, tgBot, qqClient) {
        ...
        this.commandContext = new CommandContext(instance, tgBot, qqClient, this.registry, this.permissionChecker, this.stateManager, this.replyTG.bind(this), this.extractThreadId.bind(this));
        // Initialize handlers
    ...
    async loadPluginCommands() {
        ...
        try {
            ...
            for (const pluginInfo of loadedPlugins) {
                try {
                    const context = pluginInfo.context;
     ...
     */
    convertToMessageEvent(msg, pluginLogger) {
        ...
        try {
            ...
            if (bindingState && text && !text.startsWith(this.registry.prefix)) {
                ...
                if (/^-?\d+$/.test(text.trim())) {
                    ...
                    // 执行绑定逻辑
                    const forwardMap = this.instance.forwardPairs;
                    // 检查冲突
            ...
            let replenishedReply;
            const replyToId = tgMsg.replyTo?.messageId || tgMsg.replyToMessage?.id;
            if (replyToId && (!tgMsg.replyToMessage || !tgMsg.replyToMessage.text)) {
                try {
            ...
            try {
                ...
                const channelType = tgMsg.chat?.type === 'private' ? 'private' : 'group';
    ...
    async replyTG(chatId, text, threadId) {
        try {
     ...
     */
    extractMentionedBotUsernames(tgMsg, parts) {
