...
export class CommandContext {
    ...
    replyTG;
    extractThreadId;
    constructor(instance, tgBot, qqClient, registry, permissionChecker, stateManager, replyTG, extractThreadId) {
        ...
        this.replyTG = replyTG;
        this.extractThreadId = extractThreadId;
    }
    /**
     * 回复到QQ
     */
    async replyQQ(roomId, text) {
        try {
            const msg = {
     ...
     */
    async replyBoth(msg, text, commandName) {
        ...
        if (platform === 'telegram') {
            ...
            if (pair && this.shouldReplyBothSides(pair)) {
                ...
                await this.replyTG(Number(pair.tgChatId), text, pair.tgThreadId || undefined);
            }
            else if (!pair) {
     ...
     */
    isCommandAllowed(pair, commandName) {
        ...
        // 解析命令列表
        const commands = list.split(',').map((cmd) => cmd.trim().toLowerCase());
