...
function getExecutor(ctx, instanceId) {
...
function parseGatewayChannelId(raw) {
...
function normalizeGatewayReplyId(raw) {
    const input = String(raw || '').trim();
    if (!input) {
        return undefined;
    }
...
function toGatewayMessageId(platform, raw, fallbackChatId) {
    const input = String(raw || '').trim();
    if (!input) {
        return platform === 'tg'
...
function toPluginSegments(segments) {
    return segments.map((seg) => ({
        type: seg.type,
        data: seg.data,
    }));
}
function toGatewaySegments(segments) {
    return segments.map(seg => ({
        type: seg.type,
        data: seg.data,
    }));
}
function normalizeUserId(platform, raw) {
...
function buildGatewayEvent(event) {
