...
export function getAvatarUrl(room) {
    if (!room)
        return '';
...
export function getImageUrlByMd5(md5) {
    return `https://gchat.qpic.cn/gchatpic_new/0/0-0-${md5.toUpperCase()}/0`;
}
export function getBigFaceUrl(file) {
...
export function getAvatar(room) {
    return fetchFile(getAvatarUrl(room));
}
export function isContainsUrl(msg) {
...
export function hasSupportedImageExt(name) {
