...
export function napCatForwardMultiple(messages) {
    return messages.map((it) => {
        const anyIt = it;
        const contentArray = Array.isArray(anyIt.message) ? anyIt.message : [anyIt.message];
        const validContent = contentArray.filter((elem) => elem != null);
        return {
            ...
            message: validContent.map(napCatReceiveToMessageElem).filter((elem) => elem != null),
        };
    });
}
function napCatReceiveToMessageElem(data) {
    if (!data)
        return null;
    const anyData = data;
